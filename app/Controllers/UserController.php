...
class UserController extends BaseController
{
    ...
    public function nodeqrcode($request, $response, $args)
    {
        ...
        if ($node == null) {
            ...
            if ($ctype == "SSR") {
                $ssurl = $ary['server']. ":" . $ary['server_port'].":".str_replace("_compatible","",$this->user->protocol).":".$ary['method'].":".str_replace("_compatible","",$this->$user->obfs).":". base64_encode($ary['password'])."/?obfsparam=". base64_encode($user->obfs_param)."&remarks=". base64_encode($node->name);
